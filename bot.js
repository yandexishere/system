...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['KD Clan','Knights For Ever.'];
    var i = -1;
    var j = 0;
    ...
    if(message.content.startsWith('$new')) {
        ...
        if(!args) {
            return message.channel.send('**المرجو كتآبة موضوع للتذكرة**');
                ...
                if(!support) {
                    return message.channel.send('** من فضلك قم بإنشاء رتبة اسمها `@everyone` **');
                };
            if(!ticketsStation) {
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/npR5zmN**")
